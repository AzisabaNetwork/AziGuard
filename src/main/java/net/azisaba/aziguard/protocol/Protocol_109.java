...
public enum Protocol_109 implements Protocol {
    ...
    private static final Map<Integer, Protocol_109> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_109> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_109 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_109 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_109 protocol : values()) {
