...
public enum Protocol_756 implements Protocol {
    ...
    private static final Map<Integer, Protocol_756> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_756> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_756 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_756 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_756 protocol : values()) {
