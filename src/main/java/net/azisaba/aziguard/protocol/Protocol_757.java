...
public enum Protocol_757 implements Protocol {
    ...
    private static final Map<Integer, Protocol_757> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_757> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_757 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_757 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_757 protocol : values()) {
