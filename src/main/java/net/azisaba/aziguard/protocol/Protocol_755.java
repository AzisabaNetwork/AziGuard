...
public enum Protocol_755 implements Protocol {
    ...
    private static final Map<Integer, Protocol_755> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_755> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_755 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_755 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_755 protocol : values()) {
