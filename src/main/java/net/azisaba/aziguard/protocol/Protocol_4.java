...
public enum Protocol_4 implements Protocol {
    ...
    private static final Map<Integer, Protocol_4> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_4> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_4 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_4 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_4 protocol : values()) {
