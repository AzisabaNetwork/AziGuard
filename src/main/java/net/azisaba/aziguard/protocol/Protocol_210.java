...
public enum Protocol_210 implements Protocol {
    ...
    private static final Map<Integer, Protocol_210> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_210> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_210 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_210 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_210 protocol : values()) {
