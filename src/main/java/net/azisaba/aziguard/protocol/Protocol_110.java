...
public enum Protocol_110 implements Protocol {
    ...
    private static final Map<Integer, Protocol_110> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_110> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_110 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_110 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_110 protocol : values()) {
