...
public enum Protocol_401 implements Protocol {
    ...
    private static final Map<Integer, Protocol_401> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_401> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_401 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_401 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_401 protocol : values()) {
