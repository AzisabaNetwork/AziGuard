...
public enum Protocol_336 implements Protocol {
    ...
    private static final Map<Integer, Protocol_336> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_336> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_336 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_336 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_336 protocol : values()) {
