...
public enum Protocol_108 implements Protocol {
    ...
    private static final Map<Integer, Protocol_108> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_108> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_108 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_108 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_108 protocol : values()) {
