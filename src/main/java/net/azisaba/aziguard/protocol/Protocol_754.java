...
public enum Protocol_754 implements Protocol {
    ...
    private static final Map<Integer, Protocol_754> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_754> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_754 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_754 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_754 protocol : values()) {
