...
public enum Protocol_498 implements Protocol {
    ...
    private static final Map<Integer, Protocol_498> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_498> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_498 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_498 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_498 protocol : values()) {
