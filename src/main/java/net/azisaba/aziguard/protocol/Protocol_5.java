...
public enum Protocol_5 implements Protocol {
    ...
    private static final Map<Integer, Protocol_5> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_5> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_5 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_5 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_5 protocol : values()) {
