...
public enum Protocol_316 implements Protocol {
    ...
    private static final Map<Integer, Protocol_316> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_316> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_316 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_316 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_316 protocol : values()) {
