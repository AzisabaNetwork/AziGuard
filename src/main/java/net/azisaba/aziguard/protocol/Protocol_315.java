...
public enum Protocol_315 implements Protocol {
    ...
    private static final Map<Integer, Protocol_315> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_315> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_315 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_315 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_315 protocol : values()) {
