...
public enum Protocol_340 implements Protocol {
    ...
    private static final Map<Integer, Protocol_340> CLIENTBOUND = new ConcurrentHashMap<>();
    private static final Map<Integer, Protocol_340> SERVERBOUND = new ConcurrentHashMap<>();
    ...
    @Nullable
    public static Protocol_340 getCPacket(int id) {
        return CLIENTBOUND.get(id);
    }

    @Nullable
    public static Protocol_340 getSPacket(int id) {
        return SERVERBOUND.get(id);
    }

    static {
        for (Protocol_340 protocol : values()) {
